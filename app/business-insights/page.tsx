...
export default function BusinessInsightsPage() {
  ...
  const analyticsData = useMemo(() => {
    ...
    // CORRECTED: Total value calculation - sum of (selling price * quantity) for each product
    const totalValue = allProducts.reduce((sum, product) => {
      return sum + (product.sellingPrice || 0) * Number(product.quantity);
    }, 0);
    ...
    allProducts.forEach((product) => {
      ...
      categoryMap.set(category, {
        ...
        value: current.value + (product.sellingPrice || 0) * Number(product.quantity),
      });
    });
    ...
    const topProducts = allProducts
      .sort(
        (a, b) => (b.sellingPrice || 0) * Number(b.quantity) - (a.sellingPrice || 0) * Number(a.quantity)
      )
      .slice(0, 5)
      .map((product) => ({
        name: product.name,
        value: (product.sellingPrice || 0) * Number(product.quantity),
